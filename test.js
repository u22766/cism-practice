...
check('credited option is listed first', d.querySelector('#explain .rl').classList.contains('ok'));
check('ISACA context renders when present', !w.__cism.state.qs[0].x || d.querySelector('#explain .ctx')!==null);
...
console.log('\n[7] Cards: focused sessions (bottom line + spaced-repetition flashcards)');
w.goTo('cards');
check('launcher shows deck counts', /card/.test(d.getElementById('blufMeta').textContent) && /due/.test(d.getElementById('flashMeta').textContent));
w.startCardSession('bluf');
check('bottom-line session takes over (launcher hidden, tab bar hidden)', !d.getElementById('cardLauncher').classList.contains('hidden')===false && d.getElementById('tabBar').classList.contains('hidden'));
check('first card rendered with points', visible(d,'blufCard') && d.querySelectorAll('#bcPoints li').length>=2);
const nB=w.__cism.session.items.length; w.sessionStep(1); check('next advances progress', /2 \//.test(d.getElementById('sessPos').textContent));
for(let i=1;i<nB;i++) w.sessionStep(1);
check('end-of-session summary shown', visible(d,'sessDone') && /reviewed/.test(d.getElementById('doneTitle').textContent));
w.endCardSession(); check('closing returns to launcher and restores tab bar', !d.getElementById('cardLauncher').classList.contains('hidden') && !d.getElementById('tabBar').classList.contains('hidden'));
w.startCardSession('flash');
const total=w.__cism.session.items.length; check('flash session built (due-only default)', total>0);
const firstId=w.__cism.session.items[0].i; w.flipCard(); check('card flips', d.getElementById('flashCard').classList.contains('flipped'));
...
check('got-it advances the session', w.__cism.session.idx===1 && !d.getElementById('flashCard').classList.contains('flipped'));
check('SR persisted', !!w.localStorage.getItem('cism.sr'));
const secondId=w.__cism.session.items[1].i; w.markCard(false); check('again resets to box 0, due now', w.__cism.SR[secondId].b===0 && w.__cism.SR[secondId].due<=Date.now());
w.endCardSession();
